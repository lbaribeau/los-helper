...
mob_aggro = [__Three_possible_mob_strings + r" attacks you\."]  # TODO: alert here(?)
...
#(?# wither                  = [r"You withered (.+?) for (.+?) damage\."]  # TODO: Obviously needs work)
wither                  = [r"You withered " + __three_possible_mob_strings  + r" for (?P<d>\d+) damage\."]  # TODO: Obviously needs work)
...
# SPELLS
# NOTE: See code from previous attemps in test_spells.py (other attempts at the regex)
spells1 = [(
    r"\n\r"
    r"/=== Combat Spells ======================================================\\\s+"
    ...
    r"(\|\s+1\s+(?P<black1>[A-Za-z ]+?)\|\s+)?"
    r"(\|\s+2\s+(?P<black2>[A-Za-z ].+?)\|\s+)?"
    r"(\|\s+3\s+(?P<black3>[A-Za-z ].+?)\|\s+)?"
    r"(\|\s+4\s+(?P<black4>[A-Za-z ].+?)\|\s+)?"
    r"(\|\s+5\s+(?P<black5>[A-Za-z ].+?)\|\s+)?"
    r"/===="
)]
# I guess the wildcards in "black" were clobbering "Vigor" if the character didn't know any black magic... just fixed it by making it 2 regexes
spells2 = [(
