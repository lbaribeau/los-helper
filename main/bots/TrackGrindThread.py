...
class Tracks:
    def __init__(self):
        ...
        self.FROM_AMBER_CENTRE = 7*['n']+['nw','ne','n','n','n','nw','nw','nw','nw','n','n','gate','e','n','n','n','w','n','cha']
        self.SEEKERS = self.TO_AMBER_CENTRE+['e','gardens','w','n','n','e','e','s','s','w','out','e',]+self.FROM_AMBER_CENTRE
        self.AMBER_GUARD1 = self.TO_AMBER_CENTRE+['nw','nw','w','kill_guard','e','se','se']+self.FROM_AMBER_CENTRE
        self.AMBER_GUARD2 = self.TO_AMBER_CENTRE+['nw','nw','n','nw','w','ne','n','s','sw','hill','se','s','se','se']+self.FROM_AMBER_CENTRE
...
class TrackGrindThread(GrindThread):
    ...
    def __init__(self, character, command_handler, mudReaderHandler, mud_map, starting_path=None):
        ...
        if C.level <= 3:
            self.__TOTALPATHS = 14 # Coral alley (hookers help aura at level 3), mill worker area
            # idea: casino, or look around amber for level 4/5 mobs... not sure if casino is safe tho
        elif C.level <= 6:
            self.__TOTALPATHS = 18 # Fort, mime artist, muggers
        elif C.level <= 7:
            self.__TOTALPATHS = 30 # Southern bandits, weakest guards (safer than other level 6's)
        elif C.level <= 8:
            self.__TOTALPATHS = 36 # Usher, sword swallower, Seekers, Northern bandits, dwarven field workers (should have level 6 in kill list since farm hands are just spawns)
        elif C.level <= 9:
            self.__TOTALPATHS = 44 # Lvl 8 guards x4
        elif C.level <= 10:
            self.__TOTALPATHS = 48 # Rancher sentries, west amethyst guards
        elif C.level <= 12:
            self.__TOTALPATHS = 50 # East amethyst guards
            # Todo: level 10 Amber guards, Coral guards
        elif C.level <= 14:
            self.__TOTALPATHS = 98 # Brotain, Aldo, Jerrek, Tag, Dini,...
        elif C.level <= 15:
            self.__TOTALPATHS = 111 # Gregor, Bertram Dalram, brother
            # Could do level 12 Coral guards
        elif C.level <= 16:
            self.__TOTALPATHS = 124 # Horbuk, Hurn, Tardan, Floor manager
            # Could do Coral guard groups
        else:
            self.__TOTALPATHS = 124
    ...
    def decide_where_to_go(self):
        ...
        #         return self.skip()
            ...
            return self.tracks.GUARDS5[:]
        elif self.nextpath == 49:
            return self.tracks.GUARDS6[:]
