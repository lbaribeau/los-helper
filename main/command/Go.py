...
class Go(Command):
    ...
    def wait_for_flag(self, **kwargs):
        ...
        if hasattr(self, "timed_out") and self.timed_out:
            # Skip wait for cartography if we timed out (Cartography will also time out... we probably missed the text...)
            self.cartography.timed_out=True
        else:
            # If we didn't time out, here we make sure cartography got done too
