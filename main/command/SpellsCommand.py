...
class SpellsCommand(SimpleCommand):
    # I'm not sure what I was doing here... so see SpellsCommand2
    command = 'spells'
    ...
    def init(self):
        self.regex_checker = [False, False, False, False, False] # I think I used to use many regexes and wait for all of them
        self.spells = []
    ...
    def notify(self, regex, M_obj):
        ...
        while 'none' in spells:
            spells.remove('none')
        while '|' in spells:
            magentaprint("| got parsed into spells...")
...
class SpellsCommand2(SimpleCommand):
    command = 'spells'
    ...
    def __init__(self, telnetHandler, character):
        ...
        # magentaprint(str(self.regex_cart))
        # magentaprint(str(self))
        self.regex_cart = [RegexStore.spells1, RegexStore.spells2]
        self.spells = []

    def notify(self, regex, match):
        spells=[]

        if regex in RegexStore.spells1:
            for text in ['black1', 'black2', 'black3', 'black4', 'black5']:
                if match.group(text):
                    for s in match.group(text).split(' '):
                        if s:
            ...
            for i in range(0, len(spells)):
                ...
                for spell_variable_name in dir(Spells):
                    ...
                    if not spell_variable_name.startswith('_') and spells[i].startswith(getattr(Spells, spell_variable_name)):
                        ...
                        # magentaprint("Extending: " + str(getattr(Spells, spell_variable_name)), False)
                        # spells[i] = spell_variable_name
                        if not (getattr(Spells, spell_variable_name) in self.spells):
                            self.spells.append(getattr(Spells, spell_variable_name))
                        # self.spells.append(spell_variable_name)
                        # could continue
        elif regex in RegexStore.spells2:
            for text in ['white', 'white2', 'white3', 'misc','misc2','misc3']:
                if match.group(text):
                    for s in match.group(text).split(' '):
                        if s:
                            spells.append(s.lower())
            for i in range(0, len(spells)):
                ...
                for spell_variable_name in dir(Spells):
                    ...
                    if not spell_variable_name.startswith('_') and spells[i].startswith(getattr(Spells, spell_variable_name)):
                        ...
                        # self.spells.append(spell_variable_name)
                        # could continue
            #Current buffs ie. Light, Bless, Detect-Invis, are available in ['buffs','buffs2','buffs3']:
            
            self.character.spells = self.spells
            magentaprint("Spells! " + str(self.spells))
